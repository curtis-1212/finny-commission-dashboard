...
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      // ─── Query deals: Closed Won with date_onboarded in this month ───
      // Commission is earned when the customer is onboarded (starts paying),
      // not just when the deal closes. Filter by date_onboarded range.
      const dealsFilter = {
              "$and": [
                { stage: "Closed Won" },
                { date_onboarded: { "$gte": startISO } },
                { date_onboarded: { "$lte": endISO } },
                      ],
      };
      ...
      console.log(`Exec API: got ${allDeals.length} Closed Won + onboarded deals for ${monthLabel}`);

      // ─── JS-side date filter (safety net on date_onboarded) ─────────
      const deals = allDeals.filter((deal: any) => {
              const onboardDate = getVal(deal, "date_onboarded");
              if (!onboardDate) return false;
              const d = String(onboardDate).slice(0, 10);
              return d >= startISO && d <= endISO;
      });

      if (allDeals.length !== deals.length) {
              console.warn(`Exec API: JS date filter removed ${allDeals.length - deals.length} deals`);
      }

      console.log(`Exec API: ${deals.length} deals after date filter (${startISO} -> ${endISO})`);

      // ─── Get churned people (graceful) ──────────────────────────────
      let churnedSet = new Set<string>();
        try {
                ...
                console.warn("Exec API: churn query failed -- skipping churn detection.", e);
        }
